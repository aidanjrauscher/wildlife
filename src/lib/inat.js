import { GROUPS, RADIUS_KM } from './groups';
...
function cacheKey(lat, lng, nativeOnly, group) {
  return `${lat.toFixed(3)},${lng.toFixed(3)},${nativeOnly ? 'native' : 'all'},${group}`;
}
 ...
 */
export async function fetchGroupSpecies(group, { lat, lng, nativeOnly }, signal) {
  const key = cacheKey(lat, lng, nativeOnly, group.key);
